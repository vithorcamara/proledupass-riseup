...
const WalletIcon = () => (
  <Wallet className="w-5 h-5 mr-3 group-hover:text-white transition-colors" />
);
  ...
  return (
    <aside className="w-64 h-screen bg-slate-800 text-white flex flex-col fixed top-0 left-0 z-40 shadow-lg">
      ...
      <nav className="flex-grow px-3 py-4 space-y-1.5 overflow-y-auto">
        ...
        <NavItem to="/company/employees" icon={<InscriptionsIcon />}>
          {" "}
          Colaboradores
        </NavItem>
        <NavItem to="/company/registrations" icon={<UsersIcon />}>
          {" "}
          Inscrições
        </NavItem>
        <NavItem to="/company/payments" icon={<WalletIcon />}>
          {" "}
          Pagamento
        </NavItem>
        <NavItem to="/company/AuthorizedUsers" icon={<ShieldCheckIcon />}>
