...
export default function ScholarshipHolders() {
  ...
  const handleDelete = async (bolsistaId) => {
    ...
    try {
      ...
      console.error(
        "Erro ao excluir bolsista:",
        err.response?.data || err.message,
      );
      const apiError =
        err.response?.data?.message || "Falha ao excluir bolsista.";
      setMessageModalProps({
  ...
  const confirmarCancelamento = (bolsistaId) => {
    confirmarAcao({
      ...
      message:
        "Esta ação pode afetar inscrições associadas e não pode ser desfeita.",
    ...
    <div className="p-4 md:p-6 lg:p-8 bg-gray-100 min-h-screen">
      ...
      <div className="mb-6">
        <div className="relative max-w-lg">
          {" "}
          {/* Limita a largura da barra de busca */}
      ...
      <div className="bg-white rounded-xl shadow-xl overflow-hidden">
        {isLoading && filteredBolsistas.length === 0 && (
          ...
          <div className="p-8 text-center text-gray-500">
            ...
            {!searchTerm && (
              <p className="mt-1 text-sm text-gray-500">
                Comece adicionando um novo bolsista.
              </p>
            )}
          </div>
        )}
            ...
            <table className="min-w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="th-admin hidden md:table-cell">
                    CPF (Bolsista)
                  </th>
                  <th className="th-admin hidden sm:table-cell">
                    Email (Cliente)
                  </th>
                  <th className="th-admin hidden lg:table-cell">
                    Necessidades
                  </th>
                ...
                {filteredBolsistas.map((bolsista) => (
                  ...
                  >
                    ...
                    <td className="td-admin hidden lg:table-cell">
                      <span
                        className={`px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${bolsista.needs ? "bg-orange-100 text-orange-800" : "bg-blue-100 text-blue-800"}`}
                      >
                    ...
                    <td className="td-admin text-right pr-6 space-x-1 sm:space-x-3 whitespace-nowrap">
                      <button
                        onClick={() =>
                          navigate(
                            `/admin/scholarship-holders/edit/${bolsista.id}`,
                          )
                        }
        ...
        {filteredBolsistas.length > 0 && (
          <div className="p-4 bg-gray-50 border-t border-gray-200 text-sm text-gray-500">
            Exibindo {filteredBolsistas.length} de {allBolsistas.length}{" "}
            bolsista(s)
          </div>
        )}
      </div>
