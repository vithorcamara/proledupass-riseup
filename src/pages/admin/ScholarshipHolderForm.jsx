...
export default function ScholarshipHolderForm() {
  ...
  useEffect(() => {
    ...
    if (bolsistaId) {
      setIsLoading(true);
      axiosInstance
        .get(`/scholarship-holders/${bolsistaId}`)
        .then((res) => {
          const data = res.data;
          setFormData({
        ...
        .catch((err) => {
          ...
          setError(
            "Não foi possível carregar os dados do bolsista para edição.",
          );
        })
  ...
  const handleSubmit = async (e) => {
    ...
    const apiPath = bolsistaId
      ? `/scholarship-holders/${bolsistaId}`
    ...
    try {
      ...
      console.error(
        `Erro ao ${bolsistaId ? "atualizar" : "criar"} bolsista:`,
        err.response?.data || err.message,
      );
      const apiError =
        err.response?.data?.message ||
        err.response?.data?.error ||
        `Falha ao ${bolsistaId ? "atualizar" : "criar"} bolsista.`;
        ...
        <fieldset>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 border-t border-gray-200 pt-4">
            ...
            <div>
              ...
              />
              {formErrors.cpf && (
                <p className="text-xs text-red-600 mt-1">{formErrors.cpf}</p>
              )}
            </div>
            <div>
