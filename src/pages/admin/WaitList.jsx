...
export default function Registrations() {
  ...
  const fetchInscriptions = async () => {
    ...
    try {
      ...
      setError(
        "Falha ao carregar inscrições do aguardando vagas. Tente novamente mais tarde.",
      );
      setInscriptions([]);
    } finally {
      setIsLoading(false);
  ...
  const handleSendEmail = async (customerId) => {
    try {
      ...
      const response = await axiosInstance.post(
        `/registrations/send-email/${customerId}`,
      );
      alert(response.data);
    } catch (error) {
            ...
            <table className="min-w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="th-admin px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-48">
                    Status Atual
                  </th>
                ...
                {inscriptions.map((insc) => (
                  ...
                  >
                    <td className="td-admin font-mono text-xs">
                      <Link
                        to={`/admin/registrations/${insc.id}`}
                    ...
                    <td className="td-admin text-right pr-6 space-x-1 sm:space-x-3 whitespace-nowrap">
                      <button
                        ...
                        onClick={() =>
                          handleSendEmail(
                            insc.scholarshipHolders?.customers?.id,
                          )
                        }
                      >
                        Enviar Email
                      ...
                      <button
                        onClick={() =>
                          navigate(`/admin/registrations/${insc.id}`)
                        }
